...
import { X, Send, Sparkles, Mic, Brain, Crown, Clock, Loader2 } from 'lucide-react';
...
const TransactionInput = ({ onClose, editingTransaction }: TransactionInputProps) => {
  ...
  const processWithAI_Natural = async (text: string) => {
    ...
    try {
      ...
      // Incrementa l'utilizzo nel database solo per utenti non premium
      if (!hasActiveSubscription) {
        try {
          await incrementAINaturalInput.mutateAsync();
        } catch (incrementError) {
          // Se l'incremento fallisce, significa che il limite è stato raggiunto
